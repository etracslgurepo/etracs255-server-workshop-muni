...
drop view if exists batchgr_error;
drop view if exists batchgr_forprocess;
drop view if exists batchgr_item;
drop view if exists batchgr_log;
drop view if exists batchgr_task;
drop view if exists batchgr;
...
CREATE TABLE `rptledger_tag` (
  ...
  PRIMARY KEY (`objid`),
  UNIQUE KEY `ux_rptledger_tag` (`parent_objid`,`tag`) USING BTREE,
  KEY `FK_rptledgertag_rptledger` (`parent_objid`) USING BTREE,
  CONSTRAINT `rptledger_tag_ibfk_1` FOREIGN KEY (`parent_objid`) REFERENCES `rptledger` (`objid`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8
;
