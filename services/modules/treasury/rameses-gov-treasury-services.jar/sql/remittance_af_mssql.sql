        ...
        from ( 
			...
			from ( 
				select t1.remittanceid, t1.controlid, 
					(case 
						when convert(date, d.txndate) = t1.remittancedate and d.qtyissued > 0 then null 
						when convert(date, d.txndate) = t1.remittancedate and d.qtyreceived > 0 then d.receivedstartseries 
						else null 
					end) as receivedstartseries, 
					(case 
						when convert(date, d.txndate) = t1.remittancedate and d.qtyissued > 0 then null 
						when convert(date, d.txndate) = t1.remittancedate and d.qtyreceived > 0 then d.receivedendseries 
						else null 
					end) as receivedendseries, 
					(case 
						when convert(date, d.txndate) = t1.remittancedate and d.qtyissued > 0 then 0
						when convert(date, d.txndate) = t1.remittancedate and d.qtyreceived > 0 then d.qtyreceived 
						else 0 
					end) as qtyreceived, 
					(case 
						when d.qtyissued > 0 then d.endingstartseries 
						when convert(date, d.txndate) <> t1.remittancedate then d.endingstartseries 
						when d.qtybegin > 0 then d.endingstartseries else null 
					end) as beginstartseries, 
					(case 
						when d.qtyissued > 0 then d.endingendseries 
						when convert(date, d.txndate) <> t1.remittancedate then d.endingendseries 
						when d.qtybegin > 0 then d.endingendseries else null 
					end) as beginendseries, 
					(case 
						when d.qtyissued > 0 then d.qtyending 
						when convert(date, d.txndate) <> t1.remittancedate then d.qtyending 
						when d.qtybegin > 0 then d.qtyending else 0 
					end) as qtybegin, 
					null as issuedstartseries, null as issuedendseries, 0 as qtyissued 
				from ( 
					select 
						r.objid as remittanceid, convert(date, r.dtposted) as remittancedate, 
				...
				select t2.remittanceid, t2.controlid, 
					(case 
						when convert(date, d.txndate) = t2.remittancedate and d.qtyissued > 0 then null 
						when convert(date, d.txndate) = t2.remittancedate and d.qtyreceived > 0 then d.receivedstartseries 
						else null 
					end) as receivedstartseries, 
					(case 
						when convert(date, d.txndate) = t2.remittancedate and d.qtyissued > 0 then null 
						when convert(date, d.txndate) = t2.remittancedate and d.qtyreceived > 0 then d.receivedendseries 
						else null 
					end) as receivedendseries, 
					(case 
						when convert(date, d.txndate) = t2.remittancedate and d.qtyissued > 0 then 0
						when convert(date, d.txndate) = t2.remittancedate and d.qtyreceived > 0 then d.qtyreceived 
						else 0 
					end) as qtyreceived, 
					(case 
						when d.qtyissued > 0 then d.endingstartseries 
						when convert(date, d.txndate) <> t2.remittancedate then d.endingstartseries 
						when d.qtybegin > 0 then d.endingstartseries else null 
					end) as beginstartseries, 
					(case 
						when d.qtyissued > 0 then d.endingendseries 
						when convert(date, d.txndate) <> t2.remittancedate then d.endingendseries 
						when d.qtybegin > 0 then d.endingendseries else null 
					end) as beginendseries, 
					(case 
						when d.qtyissued > 0 then d.qtyending 
						when convert(date, d.txndate) <> t2.remittancedate then d.qtyending 
						when d.qtybegin > 0 then d.qtyending else 0 
					end) as qtybegin, 
				...
				from ( 
					...
					from ( 
						...
						select 
							...
							convert(int, (sum(c.amount) / af.denomination)) as qtyissued  
						from remittance r 
							inner join cashreceipt c on c.remittanceid = r.objid 
							inner join af_control afc on afc.objid = c.controlid 
							inner join af on (af.objid = afc.afid and af.formtype <> 'serial') 
