[findBeginBalance]
select 
	convert(dr, decimal(20,4)) as dr, 
	convert(cr, decimal(20,4)) as cr, 
	convert(balance, decimal(20,4)) as balance 
from ( 
	select 
...
from (
	...
	from ( 
		select 
			...
			(case when txndate <= ifnull(remittancedtposted, txndate) then 1 else 3 end) as sortindex 
...
from ( 
	...
	from ( 
		select 
			...
			(case when c.txndate <= ifnull(c.remittancedtposted, c.txndate) then 1 else 3 end) as groupindex 
