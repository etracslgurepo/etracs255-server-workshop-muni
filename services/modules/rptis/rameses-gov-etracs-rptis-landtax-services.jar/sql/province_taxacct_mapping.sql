...
select * from ( 
	select 
		concat(xx.objid,':prov:',b.objid) as objid, 'APPROVED' as state, '-' as code, 
		concat('PROV. ', b.name , ' ' , ia.title) as title, 
		concat('PROV. ', b.name , ' ' , ia.title) as description, ia.type, 
...
select 
	b.objid as objid, 
	(select objid from itemaccount where objid=concat('basicadvance:prov:',b.objid)) as basicadvacct_objid,  
	(select objid from itemaccount where objid=concat('basicprev:prov:',b.objid)) as basicprevacct_objid,  
	(select objid from itemaccount where objid=concat('basicprevint:prov:',b.objid)) as basicprevintacct_objid,  
	(select objid from itemaccount where objid=concat('basicprior:prov:',b.objid)) as basicprioracct_objid,  
	(select objid from itemaccount where objid=concat('basicpriorint:prov:',b.objid)) as basicpriorintacct_objid,  
	(select objid from itemaccount where objid=concat('basiccurrent:prov:',b.objid)) as basiccurracct_objid,  
	(select objid from itemaccount where objid=concat('basiccurrentint:prov:',b.objid)) as basiccurrintacct_objid, 

	(select objid from itemaccount where objid=concat('sefadvance:prov:',b.objid)) as sefadvacct_objid,  
	(select objid from itemaccount where objid=concat('sefprev:prov:',b.objid)) as sefprevacct_objid,  
	(select objid from itemaccount where objid=concat('sefprevint:prov:',b.objid)) as sefprevintacct_objid,  
	(select objid from itemaccount where objid=concat('sefprior:prov:',b.objid)) as sefprioracct_objid,  
	(select objid from itemaccount where objid=concat('sefpriorint:prov:',b.objid)) as sefpriorintacct_objid,  
	(select objid from itemaccount where objid=concat('sefcurrent:prov:',b.objid)) as sefcurracct_objid,  
	(select objid from itemaccount where objid=concat('sefcurrentint:prov:',b.objid)) as sefcurrintacct_objid,

	(select objid from itemaccount where objid=concat('basicidlecurrent:prov:',b.objid)) as basicidlecurracct_objid, 
	(select objid from itemaccount where objid=concat('basicidlecurrentint:prov:',b.objid)) as basicidlecurrintacct_objid, 
	(select objid from itemaccount where objid=concat('basicidleprev:prov:',b.objid)) as basicidleprevacct_objid, 
	(select objid from itemaccount where objid=concat('basicidleprevint:prov:',b.objid)) as basicidleprevintacct_objid, 
	(select objid from itemaccount where objid=concat('basicidleadvance:prov:',b.objid)) as basicidleadvacct_objid 
from province b 
