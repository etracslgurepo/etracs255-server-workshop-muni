...
select * from ( 
	select top 100 percent 
		(xx.objid + ':muni:' + b.objid) as objid, 'APPROVED' as state, '-' as code, 
		('MUNI. ' + b.name + ' ' + ia.title) as title, 
		('MUNI. ' + b.name + ' ' + ia.title) as description, ia.type, 
...
select 
	b.objid as lguid, 
	(select objid from itemaccount where objid=('basicadvance:muni:' + b.objid)) as basicadvacct_objid,  
	(select objid from itemaccount where objid=('basicprev:muni:' + b.objid)) as basicprevacct_objid,  
	(select objid from itemaccount where objid=('basicprevint:muni:' + b.objid)) as basicprevintacct_objid,  
	(select objid from itemaccount where objid=('basicprior:muni:' + b.objid)) as basicprioracct_objid,  
	(select objid from itemaccount where objid=('basicpriorint:muni:' + b.objid)) as basicpriorintacct_objid,  
	(select objid from itemaccount where objid=('basiccurrent:muni:' + b.objid)) as basiccurracct_objid,  
	(select objid from itemaccount where objid=('basiccurrentint:muni:' + b.objid)) as basiccurrintacct_objid, 

	(select objid from itemaccount where objid=('sefadvance:muni:' + b.objid)) as sefadvacct_objid,  
	(select objid from itemaccount where objid=('sefprev:muni:' + b.objid)) as sefprevacct_objid,  
	(select objid from itemaccount where objid=('sefprevint:muni:' + b.objid)) as sefprevintacct_objid,  
	(select objid from itemaccount where objid=('sefprior:muni:' + b.objid)) as sefprioracct_objid,  
	(select objid from itemaccount where objid=('sefpriorint:muni:' + b.objid)) as sefpriorintacct_objid,  
	(select objid from itemaccount where objid=('sefcurrent:muni:' + b.objid)) as sefcurracct_objid,  
	(select objid from itemaccount where objid=('sefcurrentint:muni:' + b.objid)) as sefcurrintacct_objid,

	(select objid from itemaccount where objid=('basicidlecurrent:muni:' + b.objid)) as basicidlecurracct_objid, 
	(select objid from itemaccount where objid=('basicidlecurrentint:muni:' + b.objid)) as basicidlecurrintacct_objid, 
	(select objid from itemaccount where objid=('basicidleprev:muni:' + b.objid)) as basicidleprevacct_objid, 
	(select objid from itemaccount where objid=('basicidleprevint:muni:' + b.objid)) as basicidleprevintacct_objid, 
	(select objid from itemaccount where objid=('basicidleadvance:muni:' + b.objid)) as basicidleadvacct_objid 
from municipality b 
